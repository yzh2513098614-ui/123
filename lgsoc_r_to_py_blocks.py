...
def run_go_kegg_placeholder(gene_file: str, out_prefix: str) -> Dict[str, str]:
    """占位 GO/KEGG 导出，保证流程函数完整可调用。"""
    genes = pd.read_csv(gene_file, sep=None, engine="python")
    first_col = genes.columns[0] if not genes.empty else "Gene"
    glist = genes[first_col].astype(str).tolist() if not genes.empty else []
    go_file = f"{out_prefix}_GO_placeholder.csv"
    kegg_file = f"{out_prefix}_KEGG_placeholder.csv"
    pd.DataFrame({"Gene": glist, "GO_term": ["placeholder"] * len(glist)}).to_csv(go_file, index=False)
    pd.DataFrame({"Gene": glist, "KEGG_pathway": ["placeholder"] * len(glist)}).to_csv(kegg_file, index=False)
    return {"go": go_file, "kegg": kegg_file}


def correlate_targets_with_ic50(test_expr_file: str, pred_ic50_file: str, out_csv: str) -> pd.DataFrame:
    """药敏相关性占位实现：按同名样本计算 Spearman。"""
    te = pd.read_csv(test_expr_file, sep=None, engine="python", index_col=0)
    pi = pd.read_csv(pred_ic50_file, sep=None, engine="python", index_col=0)
    common = [c for c in te.columns if c in pi.index]
    rows = []
    if common:
        for g in te.index[: min(10, len(te.index))]:
            gx = pd.to_numeric(te.loc[g, common], errors="coerce").to_numpy()
            for d in pi.columns[: min(20, len(pi.columns))]:
                dy = pd.to_numeric(pi.loc[common, d], errors="coerce").to_numpy()
                n = min(len(gx), len(dy))
                if n < 5:
                    continue
                cor, p = stats.spearmanr(gx[:n], dy[:n])
                rows.append({"Gene": g, "Drug": d, "Cor": cor, "Pval": p})
    out = pd.DataFrame(rows)
    out.to_csv(out_csv, index=False)
    return out


def bubble_plot_gene_drug_cor(cor_file: str, out_png: str, top_n: int = 30) -> None:
    """药敏相关性气泡图（读取相关性结果文件）。"""
    cor_df = pd.read_csv(cor_file, sep=None, engine="python")
    if cor_df.empty:
        return
    if "Pval" in cor_df.columns:
        sig = cor_df[cor_df["Pval"] < 0.05].copy()
    else:
        sig = pd.DataFrame()
    if sig.empty:
        sig = cor_df.copy()
        sig["abs_cor"] = np.abs(pd.to_numeric(sig.get("Cor", 0), errors="coerce").fillna(0))
        sig = sig.sort_values("abs_cor", ascending=False).head(top_n)
    else:
        sig = sig.sort_values("Pval").head(top_n)
    x = np.arange(len(sig))
    cvals = pd.to_numeric(sig.get("Cor", 0), errors="coerce").fillna(0).to_numpy()
    fig, ax = plt.subplots(figsize=(10, 6))
    sc = ax.scatter(x, np.zeros_like(x), s=np.abs(cvals) * 280 + 20, c=cvals, cmap="coolwarm", vmin=-1, vmax=1)
    ax.set_xticks(x)
    ax.set_xticklabels([f"{g}\n{d}" for g, d in zip(sig.get("Gene", "NA"), sig.get("Drug", "NA"))], rotation=45, ha="right")
    ax.set_yticks([])
    ax.set_title("Gene-Drug Sensitivity Correlation")
    fig.colorbar(sc, ax=ax, label="Spearman r")
    fig.tight_layout()
    fig.savefig(out_png, dpi=300)
    plt.close(fig)
